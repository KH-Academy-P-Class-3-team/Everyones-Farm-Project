...
$("#delete-answer").on("click", function(){
	...
	$.ajax({
		...
		, data: "answeredNo=" + answeredNo + "&questionNo=" + questionNo + "&deleteCode=2" /* ajax 통신 데이터 */
...
// 			console.log(res.questionNo)
			...
			if(res.isDeleted != 1){
				...
				// redirect 진행
				$(location).attr("href", "/farmapp/admin/oneonone/farmer/detail?questionNo=" + res.questionNo)
			}
		}
		, error: function(){
			alert("서버통신 오류입니다.")
