			...
			<div class="top__member-desc">
				<!-- 로그인 시 에는 추후에 추가 예정 -->
					...
					<c:when test="${not empty farmerInfo }">
						<a href="<%=request.getContextPath() %>/farmer/logout">로그아웃</a><a href="<%=request.getContextPath()%>/mypage/user/modify">마이페이지</a><a href="<%= request.getContextPath() %>/farmintroduce/farmintroduceForm.do?farmerNo=${farmerInfo.farmerNo}">나의농장</a><a href="#"><i class="fas fa-shopping-cart"></i></a><a href="#"><i class="fas fa-search"></i></a>
					</c:when>
