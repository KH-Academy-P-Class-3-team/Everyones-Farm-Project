...
$(document).ready(function() {
	...
	$(".list-image").each(function() {
		if($(this).find("img").length == 0) {
			$(this).prepend($("<img src='<%=request.getContextPath() %>/resources/image/product/peach.jpg' width='270' >"))
		} 
	})
	...
	$(".btnSearch").click(function() {
		...
		xhr.send('filter=' + filter + '&title=' + title + '&isSeasonalFood=1');
...
<!-- 	</div> -->
	...
	<div class="activity-list">
		...
		<div class="row" onclick="javascript:location.href='<%=request.getContextPath()%>/product/productDetail.do?productNo=${season.productNo}'">
			...
			<div class="list-image">
				...
				<c:forEach items="${fileList }" var="file">
					<c:if test="${file.postNo eq season.productNo }">
			...
			<div class="caption">
				...
				<div class="caption-content"><b>종류: ${season.kind }</b></div>
				<div class="caption-content"><b>이용가능한 음식: ${season.food }</b></div>
			</div>
		...
		<div class="paging">
			<div class="paging-in">
				<a href="<%=request.getContextPath()%>/product/seasonlist.do" class="nav first"></a>
			</div>
			<c:choose>
				<c:when test="${paging.blockStart > 1 }">
					<div class="paging-in pre">
						<a href="<%= request.getContextPath() %>/product/seasonlist.do?cPage=${paging.blockStart-1}" class="nav prev"></a>
					</div>
				</c:when>
				<c:otherwise>
					<div class="paging-in pre">
						<a href="<%= request.getContextPath() %>/product/seasonlist.do?cPage=${paging.blockStart}" class="nav prev"></a>
					</div>
				</c:otherwise>
			</c:choose>
			<c:forEach begin="${paging.blockStart}" end="${paging.blockEnd}" var="page">
				<div class="paging-in">
					<a href="<%= request.getContextPath() %>/product/seasonlist.do?cPage=${page}" class="num active">${page}</a>
				</div>
			</c:forEach>
			...
			<c:choose>
				<c:when test="${paging.blockEnd+1 > paging.lastPage }">
					<div class="paging-in nex">
						<a href="<%= request.getContextPath() %>/product/seasonlist.do?cPage=${paging.blockEnd}" class="nav next"></a>
					</div>
				</c:when>
				<c:otherwise>
					<div class="paging-in nex">
						<a href="<%= request.getContextPath() %>/product/seasonlist.do?cPage=${paging.blockEnd+1}" class="nav next"></a>
					</div>
				</c:otherwise>
			</c:choose>

			<div class="paging-in">
				<a href="<%= request.getContextPath() %>/product/seasonlist.do?cPage=${paging.lastPage}" class="nav last"></a>
			</div>
		</div>
		<!-- // section pagination -->
